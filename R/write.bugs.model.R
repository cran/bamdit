#####################################################################
# Write the model in bugs to a tmp-file
#####################################################################

.write.bugs.model <- function(re="normal", link="logit"){
...
switch(link,
###############################################################################
# model for the logit link
logit = cat(normal.pre.link,"
    logit(tpr[i]) <- m[i,1]/2 + m[i,2]/2            # (Di + Si)/2
    logit(fpr[i]) <- m[i,2]/2 - m[i,1]/2            # (Di - Si)/2
   	", normal.post.link,
 file = .in.tmp("bamdit-model.bug")),
###############################################################################
# model for the logit complementary log link
cloglog = cat(normal.pre.link,"
...
#   cloglog(tpr[i]) <- (y[i, 1] + y[i, 2])/2      # (Di + Si)/2 with cloglog
#   cloglog(fpr[i]) <- (y[i, 2] - y[i, 1])/2      # (Di - Si)/2
